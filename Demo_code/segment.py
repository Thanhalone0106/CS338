...
image_path = 'upload_image.png'   
image = cv2.imread(image_path)  
def ve_hinh_chu_nhat_hinh_anh(image,xmin, ymin, xmax, ymax):
    # Đọc hình ản
    
    
    # xmin, ymin, xmax, ymax = map(float, values)
    blur_image=cv2.rectangle(image,(int(xmin), int(ymin)), (int(xmax), int(ymax)), (128, 128, 128), -1)
...
video_path = 'videos\VID20230222172933.mp4'
output_path = 'output_video.mp4'
# bounding_boxes = [(100, 100, 200, 200), (300, 300, 400, 400)]  # Ví dụ các bounding box

# draw_bounding_boxes_on_video(video_path, output_path)




def ve_bounding_box_tren_video(video_path, output_path, model_weights):
    # Tạo đối tượng YOLO với trọng số của mô hình
    ...
    # Biến flag để kiểm soát vòng lặp
    stop_flag = False

    # Vòng lặp qua từng frame trong video
    while video.isOpened() and not stop_flag:
        ret, frame = video.read()

        if not ret:
            break

        # Chuyển đổi màu của frame sang RGB
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        # Sử dụng mô hình YOLO để dự đoán bounding box trên frame
        ...
        for result in results:
            for box in result.boxes:
                ...
                xmin, ymin, xmax, ymax = map(float, values)
                cv2.rectangle(frame, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (128, 128, 128), -1)
...
# ve_bounding_box_tren_video(video_path, output_path, model_weights)
# image = cv2.imread(image_path)
...
#         values = re.findall(r'\d+\.\d+', coordinates_str)
#         xmin, ymin, xmax, ymax = map(float, values)
#         cv2.rectangle(img, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (128, 128, 128), -1)
